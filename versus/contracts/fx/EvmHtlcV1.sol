...
/// @notice Ownerless, single-asset conditional locks for Versus FX.
/// @dev One deployment is bound to one exact ERC-20. Off-chain capability
/// manifests admit only reviewed deployments and vanilla token behavior.
contract EvmHtlcV1 is ReentrancyGuard {
    ...
    /// @notice Funds a unique lock. The asset, payout parties, hash, and timeout
    /// cannot be changed after this call.
    function fund(
        bytes32 lockId,
        address beneficiary,
        address refundAddress,
    ...
    /// @notice Anyone may reveal the secret, but funds always go to the fixed
    /// beneficiary recorded when the lock was funded.
    function claim(bytes32 lockId, bytes32 secret) external nonReentrant {
        ...
        _transferExact(lock.beneficiary, lock.amount);
    ...
    /// @notice Anyone may submit an expired refund, but funds always go to the
    /// fixed refund address recorded when the lock was funded.
    function refund(bytes32 lockId) external nonReentrant {
        ...
        totalLocked -= lock.amount;
        _transferExact(lock.refundAddress, lock.amount);
