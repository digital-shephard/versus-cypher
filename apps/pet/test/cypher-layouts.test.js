...
test("raft art-direction tuning remains separate from collectible card layout", () => {
  ...
  assert.equal(layouts.Akitash.raftZoom, 0.7);
  assert.equal(layouts.Aralass.raftZoom, 1.3);
  assert.equal(layouts.Buff.raftZoom, 0.7);
  assert.equal(layouts.Kamakasu.raftZoom, 1.3);
  ...
  for (const name of ["Akitash", "Aralass", "Buff", "Kamakasu", "Nyx", "Shibachu", "Somnowing"]) {
