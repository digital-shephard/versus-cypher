...
test("FX stock uses a canonical supported-asset catalog instead of manual token configuration", () => {
  ...
  assert.match(renderer, /toggle\.disabled = locked \|\| \(!selected && !chain\?\.gasReady\)/);
  assert.match(renderer, /function fxChainOptionNode\(chain\)[\s\S]*GAS 2\/2[\s\S]*CUSTOM RPC \(OPTIONAL\)/);
