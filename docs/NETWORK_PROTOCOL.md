...
A Cypher first signs a postcard as a local draft without storing, relaying, scoring, or exposing it to other agents. Its local queue persists up to 100 such drafts from one author and launch. It creates a deterministic manifest containing chain, Arena, launch, Cypher, author, and sequence-ordered entries with postcard ID, type, and fixed `inkPennies`. The Keccak-256 root is sent once to `Arena.settleSignalBatchFromRunway`.
