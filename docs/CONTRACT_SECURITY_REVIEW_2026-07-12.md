...
- Seeded Hardhat stateful invariant campaigns with random multi-actor hatches, ticket changes, fee deposits, claims, transfers, graduations, and custody checks after every executed action.
- Foundry 1.7.1 stateful invariant campaigns with strict revert handling, 16,384 default and 128,000 deep calls per invariant, and explicit claim-cap and claim-accounting monitors.
...
`TrancheTreasury._indexRewards` originally represented fractional ticket entitlement in `accRewardPerTicket` and also reintroduced its apparent whole-unit difference through `rewardRemainder`. Repeated one-unit deposits across three tickets could therefore make aggregate claimable rewards exceed `tranchePot`, allowing early claims while later claims reverted on underflow. `rewardRemainder` is now reserved exclusively for real fees received before the first ticket; ordinary indexed fees are never counted twice.

A later 2,000-step stateful campaign found a second rounding path: repeatedly converting each Cypher's accumulator delta into whole USDC units across uneven ticket changes could make the sum of view-layer liabilities exceed the pot by tens of micro-USDC units. The final design keeps reward debt and settled entitlement in `1e27`-scaled units and performs the whole-unit floor only at claim time. The sum of executable `claimable` values therefore remains at or below `tranchePot`, while fractional entitlement carries forward. `claim` also retains a pot cap as a never-expected custody circuit breaker and emits `ClaimCapped` if that stronger invariant ever regresses. The original failing seed, random campaigns, exact vault credits, token-balance deltas, and multiple claim orders are all executable regressions.

The independent Foundry handler exercises four simulated owners and dynamically created Cyphers across hatch, referral, replenish, commit, rain, signal, fee deposit, claim, NFT transfer, graduation, referral funding, withdrawal, and time-warp actions. Both the default 16,384-call and deep 128,000-call profiles pass all three custody invariants with zero reverts or discarded calls. The strict harness also confirms that `ClaimCapped` is never reached and that every successful claim exactly matches Treasury pot, Treasury USDC, and NFT-vault balance changes. Foundry is the high-volume engine; the seeded Hardhat campaign remains a fast regression and exact-counterexample harness.
...
- Treasury fixed-point division is deliberate. Per-Cypher fractional entitlement carries forward in scaled storage, global index dust remains excess custody, and `rewardRemainder` represents only real fees received before the first ticket.
