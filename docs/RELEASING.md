...
Apple can leave a first notarization submission in progress longer than GitHub's six-hour hosted-runner job limit. The macOS job therefore uploads the exact signed app and submission ID before it begins polling. If the job ends with `Apple notarization remains in progress`, use **Re-run failed jobs** on that same workflow run. The next attempt restores the signed app and continues polling the existing Apple submission; it must not rebuild or submit a duplicate. Repeat only while Apple reports `In Progress`.

The status poll retries temporary runner or Apple network failures without creating a new submission. If the workflow itself must be changed while a submission is pending, manually run the updated workflow from `main` and set `macos_notarization_run_id` to the original GitHub Actions run ID. That replacement run restores `macos-notarization-state` from the original run and resumes the same Apple submission. Never use this input with an unrelated run. A rejected submission remains a release failure and requires inspection of its notarization log.
